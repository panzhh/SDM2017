...
In the graph wavelet anomaly detection approach, there are two main parameters, which are the scale $J$ and wavelet threshold $w_{th}$. Figures~\ref{fig:scale-S-wth} illustrate the performance of graph anomaly detection by varying $J$ and $w_{th}$, respectively. The performance for different values of $J$ and $w_{th}$ is relatively stable.
